...
export default {
  ...
  data() {
    return {
      ...
      tableData: [
        ...
        { key: "孔名称", value: "" },
        { key: "孔位边距误差上限(mm)", value: "" },
        { key: "孔位边距误差下限(mm)", value: "" },
        { key: "间距误差上限(mm)", value: "" },
        { key: "间距误差下限(mm)", value: "" },
        { key: "垂直度误差上限(°)", value: "" },
        { key: "垂直度误差下限(°)", value: "" },
        { key: "孔径误差上限(mm)", value: "" },
        { key: "孔径误差下限(mm)", value: "" },
        { key: "材料上孔及窝表面Ra粗糙度要求(um)", value: "" },
        { key: "锪窝深度偏差上限(mm)", value: "" },
        { key: "锪窝深度偏差下限(mm)", value: "" },
        { key: "锪窝角度偏差上限(°)", value: "" },
        { key: "锪窝角度偏差下限(°)", value: "" }
      ]
    };
  ...
  methods: {
    ...
    getdata() {
      this.$http({
        url: this.$http.adornUrl("hdym/holeaccuracy/getLast"),
        method: "get"
      }).then(({ data }) => {
        console.log("制孔工艺要求表:", data);
        let k = 0;
        let item = data;
